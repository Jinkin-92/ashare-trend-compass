...
**新图时代（2026-08-13 阈值重拟合后，DB 库存值口径）：**

| 指标 | 2026-08-10 | 2026-08-12 |
|---|---|---|
| 温度完全一致率 | 86.3% | 90.2% |
| 温度 ±1 档率 | 98.0% | 98.0% |
| RS 偏差（快窗） | 均值 +1.6 / MAE 12.0 | 均值 +5.2 / MAE 18.4 |

**7 日综合拟合（回放口径，417 样本）**：温度一致 22.8%→42.9%，±1档 61.2%→79.9%，signed −0.99→−0.41；RS 快窗 MAE 13.5 全 7 日稳定优于 IBD 24.7。

2026-08-13 阈值重拟合（见 `2026-08-12.md`）：温 20→30、平/凉 −35→−65、凉/寒 −60→−80、寒/冻 −85→−95，沸/热不变。依据：温度 signed 偏差 7/7 参考日全负（本地系统性偏冷），跨新旧图时代方向一致，非单日噪声。
注意：新旧图温度语义不同（旧图默认档「温」，新图默认档「平」），逐日绝对一致率不可跨时代比较，但偏差方向跨时代有效。

## 已知偏差模式

| 类型 | 表现 | 根因 | 修正方向 |
|---|---|---|---|
| RS | IBD 口径 [21/63/126/252] 在反转行情响应慢（通信设备 97 vs 参考 2 等） | 长窗口滞后 | **已切换快窗口 [10/21/63/126]（2026-08-11，4 日回放 MAE 25.7→12.8）** |
| 温度 | 本地系统性偏冷约 1 档；领涨板块到不了热/沸 | 回撤因子新高时贡献恒为 0（raw 上限 0.8）+ 负向拉伸 2.0 过度放大冷侧 | **已修复（2026-08-12 方案 C）**：回撤→250日区间位置 + 负向拉伸回 1.5；08-10 一致率 29.4→41.2%，±1档 72.5→86.3% |
| 温度 | 方案 C 后仍残余系统性偏冷（7/7 参考日 signed 全负） | 分档阈值整体偏紧，参考「平」档位远宽于旧映射 | **已修复（2026-08-13 阈值重拟合）**：温 20→30、平/凉 −35→−65、凉/寒 −60→−80、寒/冻 −85→−95；新两日一致率 45.1%→88.2% |
| 温度 | 旧参考日（≤08-04）温度分布与本地公式不可比 | 趋势动物改语义：旧图默认档「温」→新图默认档「平」 | 旧参考日只用于 RS（排名口径不变），温度阈值从新图重新累计 |
| 映射 | 新版分类为聚合口径（金属钢铁/游戏传媒/家电家居等 12 个） | 趋势动物跨申万行业聚合 | 近似映射到代表性行业，见 calib_compare.py ALIAS |
| 个股 | 校准图含个股（旧图） | 个股不在申万 L1/L2 里 | 后续扩展个股 symbol_id 映射 |
